...
use capsules::radio_debug::RadioDebug;
...
pub unsafe fn reset_handler() {
    ...
    // Create a radio debug device that wraps the RF233 and prints
    // transmitted payloads to the console
    let rf233_debug: &RadioDebug<'static, RF233Device> =
        static_init!(RadioDebug<'static, RF233Device>,
                     RadioDebug::new(rf233));
    ...
    // Radio capsule with debug
    let radio_capsule = static_init!(
        capsules::radio::RadioDriver<'static, RadioDebug<'static, RF233Device>>,
        capsules::radio::RadioDriver::new(rf233_debug));
    radio_capsule.config_buffer(&mut RADIO_BUF);
    rf233_debug.set_transmit_client(radio_capsule);
    rf233_debug.set_receive_client(radio_capsule, &mut RF233_RX_BUF);
    rf233_debug.set_config_client(radio_capsule);
