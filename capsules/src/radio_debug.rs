...
impl<'a, R: radio::Radio + 'a> RadioDebug<'a, R> {
    pub fn new(radio: &'a R) -> RadioDebug<'a, R> {
...
impl<'a, R: radio::Radio + 'a> radio::RadioConfig for RadioDebug<'a, R> {
    fn initialize(&self,
                  buf: &'static mut [u8],
...
impl<'a, R: radio::Radio + 'a> radio::RadioData for RadioDebug<'a, R> {
    ...
    fn transmit(&self,
        ...
        let offset = self.radio.payload_offset(source_long, false) as usize;
    ...
    fn transmit_long(&self,
                     ...
                     -> ReturnCode {
        // Transmit len is header + payload
        let offset = self.radio.payload_offset(source_long, true) as usize;
        let payload_len = (len - self.radio.header_size(source_long, true)) as usize;
        debug!("TX packet: \
               (long) dst={:02x}{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}, \
               len={}, src_long={}, payload_len={}",
