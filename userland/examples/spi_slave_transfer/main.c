...
// master and the slave. Further, after we receive the buffer with data,
// we check to make sure we received the correct values. If not, we enable
// the LED and never disable it.
int main(void) {
  int i;
  for (i = 0; i < BUF_SIZE; i++) {
    wbuf[i] = 0;
